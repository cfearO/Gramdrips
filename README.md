...
- `POST /api/withdraw` *(Bearer token, or `{ wallet }` in DEV_MODE)* — **ledger-only stub**, see below. `400 no_withdrawal_wallet_linked` or `400 below_threshold` if not ready
...
- **Attach a real persistent volume** (Railway) or a paid persistent disk
  ...
  either, so this matters more the more traffic you get.
...
- **Fund and sign withdrawals** — `/api/withdraw` only tracks off-chain
  balances and zeroes the bucket; it does not send GRAM. Actually paying out
  is a separate, security-sensitive piece (hot wallet management, withdrawal
  limits, etc.) not included here.
