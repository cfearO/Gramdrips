...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    // LEDGER-ONLY STUB — see README "Before this touches real money". This
    // zeroes the bucket and records the withdrawal in db.json; it does not
    // move any real GRAM. Wire in an actual signed TON transfer here before
    ...
    if (req.method === 'POST' && url.pathname === '/api/withdraw') {
      ...
      const result = await withDb((db) => {
        ...
        const withdrawal = { wallet, amount: balance, to: user.withdrawalWallet, at: new Date().toISOString() };
        db.withdrawals.push(withdrawal);
        return { ok: true, status: 200, withdrawal, status_after: shapeStatus(db, wallet, resolved.verified) };
      });
      ...
      return send(res, 200, {
        ...
        note: 'Ledger-only stub — no on-chain transfer has been sent. See backend/README.md.',
      });
    }
